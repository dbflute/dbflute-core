...
public class TnBeanListResultSetHandler extends TnAbstractBeanResultSetHandler {
    ...
    protected boolean checkNonSpecifiedColumnAccess(boolean hasCB, boolean hasOql) {
        if (hasCB) {
            ...
            // CustomizeEntity does not need to check because it is generated by select clause
            // while the check is no risk so checkable but just in case...
            // and basically not allowed, but context has determination for compatible option
            return DomainEntity.class.isAssignableFrom(resultType) && !context.isNonSpecifiedColumnAccessAllowed();
        }
        return false;
    }
